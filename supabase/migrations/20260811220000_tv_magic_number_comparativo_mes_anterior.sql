-- =============================================================================
-- TV ReATIVA — "Magic Number" vira COMPARATIVO com o mês passado
-- -----------------------------------------------------------------------------
-- A meta 'magic' deixa de ser o valor fixo de R$ 500.000 e passa a ser
-- "Superar o mês passado": alvo = honorários do MÊS ANTERIOR (pagamentos), e o
-- realizado continua sendo o honorário do mês corrente. Se não houver mês
-- anterior, cai no fallback histórico de 500.000 (evita meta "batida" indevida).
-- Fiel à função vigente (20260811200000), com 3 acréscimos: declaração v_hon_ant,
-- cálculo do honorário do mês anterior e a linha 'magic' do array de metas.
-- =============================================================================
...
declare
  ...
  v_premiacao jsonb;
  v_rec bigint; v_hon bigint; v_hon_ant bigint; v_hon_total bigint; v_taxa_dia numeric;
...
begin
  ...
  -- Honorário do MÊS ANTERIOR (base do comparativo "Superar o mês passado").
  select coalesce(round(sum(valor_honorario)),0)::bigint into v_hon_ant
  from public.pagamentos where data_pagamento is not null
    and date_trunc('month',data_pagamento) = date_trunc('month',current_date) - interval '1 month';
  ...
  v_mes_obj := jsonb_build_object(
    ...
    'honorarios_mes_anterior', v_hon_ant,
