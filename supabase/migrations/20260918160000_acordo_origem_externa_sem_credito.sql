...
-- 18/09/2026.
--
-- O CASO: pagamento 71803. O operador do arquivo (OSVALDINA.ALVES) nao e usuario
-- do CRM; a previa recusa por OPERADOR_CADASTRADO, e mesmo que passasse, o
-- gatilho `_acordo_herda_responsavel_do_aluno` completaria o responsavel vazio
-- com o dono atual do aluno -- credito indevido.
--
-- A REGRA: continua BLOQUEADO por padrao. A excecao so existe quando a gestao
-- autoriza AQUELE pagamento (tabela pagamento_origem_externa_autorizada, escrita
-- so por pagamento_autorizar_origem_externa, portao de gestao). Com a linha:
--   - a previa aceita OPERADOR_CADASTRADO se o operador do arquivo nao tem
--     e-mail, nao esta no CRM e o nome bate com o autorizado;
--   - o registrador cria o acordo com responsavel NULO, marca a origem na
--     observacao e grava na auditoria: origem externa, operador de origem,
--     credito NAO_ATRIBUIDO e quem autorizou;
--   - o gatilho de heranca nao preenche o responsavel -- so com a chave
--     transacional que o registrador acende E o registrador na pilha.
-- Nao muda: responsavel atual do aluno, usuarios, credito de outros acordos, a
-- regra para operador cadastrado. Outro pagamento nao herda a autorizacao.
--
-- Rollback: supabase/rollbacks/20260918160000_acordo_origem_externa_sem_credito.rollback.sql

-- ---------------------------------------------------------------------------
-- 1. A autorizacao, por pagamento
-- ---------------------------------------------------------------------------
create table if not exists public.pagamento_origem_externa_autorizada (
  pagamento_id uuid primary key references public.pagamentos(id),
  operador_origem text not null,
...
alter table public.pagamento_origem_externa_autorizada enable row level security;
revoke all on public.pagamento_origem_externa_autorizada from anon, authenticated;

create or replace function public.pagamento_autorizar_origem_externa(p_pagamento_id uuid, p_motivo text)
 returns jsonb
 language plpgsql
 security definer
...
begin
  ...
  if not coalesce(public.usuario_e_gestao(), false) then
    raise exception 'Autorizar origem externa é decisão da gestão financeira.' using errcode = '42501';
  end if;
  ...
  v_nome := btrim(coalesce(v_pag.operador_nome, ''));
  ...
  if v_pag.operador_email is not null or v_nome = '' then
    return jsonb_build_object('ok', false, 'erro', 'NAO_E_ORIGEM_EXTERNA',
      'detalhe', 'so pagamento sem e-mail de operador e com nome de operador na origem');
  end if;
  if exists (select 1 from public.usuarios u
              where lower(u.email) = lower(v_nome)
                 or lower(split_part(u.email, '@', 1)) = lower(v_nome)
                 or lower(btrim(coalesce(u.nome, ''))) = lower(v_nome)
                 or lower(btrim(coalesce(u.nome_exibicao, ''))) = lower(v_nome)) then
    return jsonb_build_object('ok', false, 'erro', 'OPERADOR_EXISTE_NO_CRM',
      'detalhe', 'o nome de origem corresponde a um usuario: use o cadastro, nao a excecao');
  end if;

  insert into public.pagamento_origem_externa_autorizada (pagamento_id, operador_origem, motivo, autorizado_por)
  values (p_pagamento_id, v_nome, btrim(p_motivo), coalesce(nullif(v_email, ''), 'gestao'))
  on conflict (pagamento_id) do nothing;
  if not found then
    return jsonb_build_object('ok', true, 'ja_autorizado', true);
  end if;

  insert into public.auditoria (usuario, acao, tabela_afetada, registro_id, detalhes)
  values (coalesce(nullif(v_email, ''), 'gestao'), 'ORIGEM_EXTERNA_AUTORIZADA', 'pagamentos', p_pagamento_id,
          jsonb_build_object('pagamento_id', p_pagamento_id, 'operador_origem', v_nome,
                             'credito_individual', 'NAO_ATRIBUIDO', 'motivo', btrim(p_motivo)));

  return jsonb_build_object('ok', true, 'pagamento_id', p_pagamento_id, 'operador_origem', v_nome,
                            'credito_individual', 'NAO_ATRIBUIDO');
end;
$fn$;
revoke all on function public.pagamento_autorizar_origem_externa(uuid, text) from public, anon;
grant execute on function public.pagamento_autorizar_origem_externa(uuid, text) to authenticated;

comment on function public.pagamento_autorizar_origem_externa(uuid, text) is
  'Gestao autoriza UM pagamento de operador de origem externa (sem e-mail, fora do CRM) a registrar acordo a vista sem credito individual. Sem esta linha a previa continua recusando por OPERADOR_CADASTRADO.';

-- ---------------------------------------------------------------------------
-- 2. A previa: OPERADOR_CADASTRADO aceita a origem externa autorizada
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. O registrador: acordo sem credito individual quando a origem e externa
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. A heranca de responsavel respeita o acordo sem credito
-- ---------------------------------------------------------------------------
