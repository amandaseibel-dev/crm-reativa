-- Correcao de 20260805300000: os titulos do aluno nao ficavam 'quitada'/zerados
-- quando estavam vinculados a um acordo. A RPC atualizava `acordos`->QUITADO
-- ANTES dos titulos; o trigger titulos_por_status_acordo ja flipava os titulos
-- vinculados para situacao='PAGO' (so a situacao). Como o UPDATE dos titulos
-- filtrava por situacao in ('ABERTO','NEGOCIADO'), esses titulos deixavam de
-- casar e ficavam status='vinculada' com saldo intacto -> continuavam
-- elegiveis/contando. Agora zeramos por STATUS (nao por situacao), pegando
-- todo titulo do aluno que nao esteja ja quitada/cancelada, independentemente
-- do que o trigger fez com a situacao.
...
begin
  ...
  -- Quita os acordos ATIVOS (o trigger trg_titulos_por_status_acordo pode
  -- flipar a situacao dos titulos vinculados para PAGO -- por isso o UPDATE
  -- de titulos abaixo filtra por STATUS, nao por situacao).
  ...
  -- Quitacao total INTENCIONAL dos titulos do proprio aluno: zera saldo e marca
  -- quitada. Filtra por STATUS (nao por situacao) para pegar tambem os que o
  -- trigger acabou de flipar para situacao='PAGO' mas seguem status='vinculada'
  -- com saldo. Assim NENHUM titulo do aluno fica elegivel a vinculo em acordo.
  update public.acordos_titulos t
  set situacao='PAGO', status='quitada', saldo_corrigido=0, valor_em_aberto=0, atualizado_em=now()
  where t.aluno_id = p_aluno_id
    and lower(coalesce(t.status,'')) not in ('quitada','cancelada','cancelado');
