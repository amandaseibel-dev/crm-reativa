-- CORREÇÃO do escopo da Amanda ADM na RPC public.projecao_dashboard.
--
-- O ajuste anterior (migration 20260725120000) ampliou indevidamente a
-- projeção da Amanda ADM para a filial inteira (ranking + histórico da
-- empresa toda). Regra correta: a Amanda ADM (cobranca07) deve enxergar
-- SOMENTE os pagamentos atribuídos a ela -- ou seja, os pagamentos cujo
-- campo pagamentos.operador_email = 'cobranca07@aelbra.com.br' (único vínculo
-- existente entre pagamento e responsável; não há campo separado de
-- "responsável administrativo"/"usuário selecionado").
--
-- Esta migration REMOVE cobranca07 do escopo administrativo geral:
--   * ranking/maior pagamento voltam a ser calculados só para a gestão
--     (amanda.seibel / cobranca04) -- como antes do PR #51;
--   * histórico dia a dia da Amanda ADM volta a ser o dela (por operador_email).
-- Ninguém mais é afetado. Metas, comissão, fórmulas, ranking, pagamentos,
-- responsáveis e dados financeiros permanecem inalterados.
--
-- MANTÉM o parâmetro opcional p_unidade (default NULL = "Todos"): para a
-- Amanda ADM, "Todos" = todos os pagamentos DELA; ao escolher uma unidade,
-- filtra apenas dentro dos pagamentos dela. Quando p_unidade IS NULL o
-- comportamento é idêntico ao estado anterior ao PR #51.
--
-- Idempotente: CREATE OR REPLACE (assinatura já é (text,text,text) em prod).
-- Rollback (restaura a versão do PR #51) em:
--   supabase/rollbacks/20260725130000_projecao_amanda_adm_somente_pagamentos_dela_down.sql
