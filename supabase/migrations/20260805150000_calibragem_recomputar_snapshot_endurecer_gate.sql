-- ============================================================================
-- Bloco 1 — Endurecimento do gate técnico de calibragem_recomputar_snapshot,
-- no mesmo padrão de calibragem_simular/simular_ano. auth.jwt() IS NULL deixa de
-- autorizar isoladamente: exige gestão (JWT) OU service_role OU sessão interna
-- (postgres/cron) via session_user técnico. Corpo inalterado (só o gate).
-- ============================================================================
...
begin
  ...
  agg as (
    select b.operador_email as op_email, max(b.operador_nome) as op_nome, count(*) as cpfs_qtd,
      ...
      count(*) filter (where b.status_acionamento is null) as sem_acion_q, sum(b.saldo_total) filter (where b.status_acionamento is null) as sem_acion_v,
