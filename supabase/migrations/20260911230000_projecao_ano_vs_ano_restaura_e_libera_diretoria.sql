-- ============================================================================
-- ANO VS ANO — restauração do payload + liberação para a Diretoria
-- ----------------------------------------------------------------------------
-- CORREÇÃO DE ERRO INTRODUZIDO EM 11/09/2026: numa tentativa de liberar a aba
-- para a Diretoria, a função foi reescrita com base em leitura parcial e as
-- chaves do retorno mudaram (meses/total_atual no lugar de por_mes/total_atu/
-- hon_total_*/mesmo_periodo). ComparativoAnoAno.jsx lê as chaves originais —
-- a aba quebrou em produção para a Amanda e a Fernanda.
--
-- Aqui o corpo volta VERBATIM ao da migration que criou a função
-- (20260811120000_projecao_ano_vs_ano.sql). A única diferença é o gate, que
-- passa a aceitar também o perfil `diretoria` (Angela e Gustavo).
--
-- Por que o gate continua no backend: é SECURITY DEFINER, então nem front nem
-- devtools burlam. Diretoria vê o comparativo consolidado da empresa; meta,
-- ranking por operador e importação continuam fora — dependem de `e_gestao`,
-- que é falso para o perfil diretoria.
--
-- Só leitura. Reversível: reaplicar 20260811120000 sem a cláusula de diretoria.
-- ============================================================================
...
declare
  ...
  v_resultado json;
begin
  if v_email not in ('amanda.seibel@aelbra.com.br','cobranca04@aelbra.com.br')
     and not coalesce(public.usuario_e_diretoria(), false) then
...
revoke all on function public.projecao_ano_vs_ano() from public, anon;
