...
-- v2 (performance): desliga o gatilho de teto durante a execução (DDL transacional:
-- se a função falhar, o rollback reativa), grava auditoria em LOTE (sem loop) e usa
-- statement_timeout 300s. Isolado: não afeta os outros fluxos de calibragem.
-- Two-phase, advisory lock, auditoria append-only, gate gestão. Aplicada em prod
-- via MCP em 2026-08-07. 1ª execução real: sim a53677d6 (2607 movs, 100% responsável
-- sincronizado; operadores 50-314 -> 513-534, saldos ~R$2,1-2,3M). Backup em
-- _backup_niv_casos_20260807 / _backup_niv_alunos_20260807.
...
begin
  ...
  alter table public.casos disable trigger trigger_impor_teto_operador;
  ...
  alter table public.casos enable trigger trigger_impor_teto_operador;
