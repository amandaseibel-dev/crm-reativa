-- ============================================================================
-- Fechamento de Remuneracao: override de gestao (recorrente) + base Diego julho
-- ----------------------------------------------------------------------------
-- Decisao Amanda 2026-08-04:
--   * Amanda gestora (amanda.seibel): 3% sobre o HONORARIO TOTAL do mes.
--   * Fernanda (cobranca04):          1,5% sobre o HONORARIO TOTAL do mes.
--     -> FIXOS TODOS OS MESES (regra recorrente embutida no calculo).
--   * Diego (cobranca13): 5% sobre o proprio honorario -- SO EM JULHO/2026
--     (lancado como config pontual de competencia, nao entra aqui no codigo).
-- A gestao deixa de ser "fora da remuneracao": passa a receber o override
-- calculado sobre o honorario total do mes (v_all_hon), somado ao total a pagar.
-- Base = honorario total do mes (todos os pagamentos, incl. sem-operador).
-- ============================================================================
...
begin
  ...
  for r in
    with prod as (
      ...
      where data_pagamento >= v_ini and data_pagamento < v_prox and operador_email is not null
  ...
  loop
    -- Gestao (Amanda gestora, Fernanda) NAO entra na regra operacional: e
    -- remunerada por override sobre o honorario TOTAL do mes, calculado apos
    -- o loop. Pula aqui (nao vira "nao elegivel" nem beneficiario operacional).
    if r.email = any(v_gestao) then
      continue;
    end if;
  ...
  select jsonb_build_object('qtd', count(*), 'valor_recuperado', coalesce(sum(valor_pago),0),
    ...
    into v_sem from public.pagamentos
  ...
  -- ------------------------------------------------------------------------
  -- Override de GESTAO (recorrente, todo mes): Amanda gestora 3% e Fernanda
  -- 1,5% sobre o HONORARIO TOTAL do mes (v_all_hon). Somado ao total a pagar.
  -- Nao afeta t_hon (honorario e producao operacional, nao base da gestao).
  -- ------------------------------------------------------------------------
  ...
  loop
    v_comissao := round(coalesce(v_all_hon,0) * r.pct / 100.0, 2);
    ...
    v_total_final := v_comissao + coalesce(v_prem,0) + coalesce(v_bonus,0)
    ...
    v_benef := v_benef || jsonb_build_object('email', r.email, 'nome', r.nome,
      'tipo_vinculo', 'gestao', 'nome_exibicao_fixo', null, 'valor_fixo', 0,
    ...
    t_com := t_com + v_comissao; t_prem := t_prem + coalesce(v_prem,0);
