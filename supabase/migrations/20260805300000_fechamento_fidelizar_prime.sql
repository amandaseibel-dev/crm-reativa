-- ============================================================================
-- Fechamento: FIDELIZAR o Prime como base da comissao.
-- ----------------------------------------------------------------------------
-- Decisao Amanda 2026-08-05: o valor do Prime (Ulbra) e sempre o que vale.
-- Quando existe Conferencia Prime do mes (fechamento_conferencia_prime), o
-- calcular_fechamento_remuneracao passa a usar o HONORARIO DO PRIME como base
-- da comissao (por operador e da gestao), no lugar do calculado ao vivo dos
-- pagamentos. Sem conferencia do mes => calcula ao vivo (e passa a fidelizar
-- sozinho assim que a conferencia for rodada). Fonte = ultima conferencia do mes.
-- Preserva: salario/premiacao do DRE, truncamento, override de gestao, e a
-- exibicao da base recuperado/honorario total-empresa na gestao.
-- ============================================================================
...
declare
  ...
  v_tem_prime boolean := false; v_prime_ops jsonb; v_prime_total_hon numeric;
...
begin
  ...
  -- FIDELIZACAO PRIME: base da comissao vem do Prime quando ha conferencia do mes.
  select total_prime_honorario, resumo->'resumo_por_operador'
    into v_prime_total_hon, v_prime_ops
  from public.fechamento_conferencia_prime
  where competencia = v_ini order by criado_em desc limit 1;
  v_tem_prime := found;
  v_fonte := case when v_tem_prime then 'PRIME (fidelizado)' else 'sistema (ao vivo)' end;
  ...
  loop
    ...
    -- Base honorario/recuperado: PRIME (fidelizado) se houver conferencia; senao ao vivo.
    if v_tem_prime then
      select (e->>'prime_honorario')::numeric, (e->>'prime_valor')::numeric into v_hon_prime, v_val_prime
      from jsonb_array_elements(v_prime_ops) e where lower(e->>'operador_email') = r.email;
      v_hon_base := coalesce(v_hon_prime, 0); v_rec_base := coalesce(v_val_prime, r.recuperado);
    else
    ...
    v_benef := v_benef || jsonb_build_object('email', r.email, 'nome', coalesce(v_nome_dre, r.nome, r.email),
      ...
      'falta_proxima_faixa', v_falta, 'situacao', v_situacao, 'honorario_fonte', v_fonte);
  ...
  -- Base da gestao: total do Prime quando fidelizado; senao total ao vivo.
  v_base_gestao_hon := case when v_tem_prime then coalesce(v_prime_total_hon,0) else coalesce(v_all_hon,0) end;
  ...
  loop
    ...
    v_benef := v_benef || jsonb_build_object('email', r.email, 'nome', coalesce(v_nome_dre, r.nome),
      ...
      'qtd_pagamentos', v_all_qtd, 'valor_recuperado', v_all_rec, 'honorarios', v_base_gestao_hon,
  ...
  return jsonb_build_object('competencia', v_mes, 'periodo_inicio', v_ini, 'periodo_fim', v_fim,
    'faixas_configuradas', v_tem_faixa, 'honorario_fonte', v_fonte, 'prime_fidelizado', v_tem_prime,
