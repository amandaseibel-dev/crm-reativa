-- ============================================================================
-- Calibragem: restringir a base do snapshot aos casos OPERACIONAIS.
--
-- Antes, a CTE base pegava todos os casos com operador (where operador_email is
-- not null), incluindo encerrados (cancelado/quitado/juridico/sem saldo). Isso
-- inflava TODOS os indicadores por operador e não batia com a carteira que o
-- operador enxerga (que filtra caso_encerrado_operacional).
--
-- Agora a base exclui encerrados via public.caso_encerrado_operacional(...).
-- Afeta todos os indicadores (contagens/saldos), alinhando a Calibragem à
-- carteira operacional. Mantém a correção anterior de "sem acionamento" por
-- data_ultimo_acionamento (migration 20260806160000). Gate técnico inalterado.
-- ============================================================================
