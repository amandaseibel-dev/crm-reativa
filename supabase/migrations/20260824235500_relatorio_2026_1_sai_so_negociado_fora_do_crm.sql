-- Ajuste do corte do relatório "Mensalidades 2026/1 sem negociação".
--
-- A migration anterior (20260824235000) tirava do relatório TODO CPF presente
-- no portador 166 da Prime. Isso derrubava o saldo de R$ 11.706.185,71 para
-- R$ 3.215.440,71 -- 70% do semestre saía de uma vez.
--
-- DECISÃO DA AMANDA (2026-08-24): sai apenas quem negociou pela Prime **e não
-- tem acordo ativo nem confirmação de pagamento registrados no CRM**.
--
-- O critério é o nível de prova, não o tamanho do número. Para esse grupo a
-- liquidação foi verificada título a título: 60 CPFs sorteados, 177 títulos de
-- 2026/1, todos os 177 liquidados na Prime (R$ 172.630,41 de R$ 172.630,41).
-- Para quem tem acordo ATIVO no CRM essa prova não existe -- o acordo pode
-- cobrir dívida de outro período e deixar as mensalidades de 2026/1 realmente
-- em aberto. Na dúvida, o título continua contado como sem negociação.
--
-- Efeito: 3.251 -> 2.853 CPFs, 9.595 -> 6.537 mensalidades,
--         R$ 11.706.185,71 -> R$ 8.192.910,10.
-- Espelho (`negociado_166`): 1.198 CPFs, 3.080 mensalidades, R$ 3.369.721,42.
--
-- A tela não muda de formato: mesmas chaves no JSON. A chave `negociado_166`
-- alimenta a segunda linha de cards, que mostra o que saiu -- sem ela a queda
-- do saldo se lê como recuperação, quando a dívida apenas virou parcela.
--
-- APLICADA EM PROD em 2026-08-24 (nome: relatorio_2026_1_sai_so_negociado_fora_do_crm).
-- Este arquivo existe para o repositório refletir produção.
...
-- A RPC da tela: mesmas chaves de sempre, mais `negociado_166` (o espelho).
-- Régua de meses de 1 a 5; o componente usa `meses.length`, então o gráfico e
