...
alter table public.fila_pagamento_sem_vinculo
  ...
  -- TERCEIRA coluna, e so por uma razao: sem ela o disparador do caminho ao
  -- vivo chamaria o MESMO caso a cada hora, para sempre, quando a Prime nao
...
as $fn$
declare
  v_pag record; v_cpf text; v_cpf_aluno text; v_n int := 0; v_aluno uuid; v_vinculou boolean := false;
  v_r jsonb;
begin
  ...
  -- CONTRATO COM A EDGE, CONFERIDO AQUI. A `prime-portador` grava a linha do 166
  -- no espelho ANTES de chamar esta RPC. Se a linha nao esta la, a evidencia nao
  -- tem lastro -- e aceitar a palavra do chamador seria deixar qualquer um marcar
  -- negociacao comprovada. A RPC tem EXECUTE para `authenticated`.
  if not exists (select 1 from public.prime_portador_membro m
                  where m.portador = 166
                    and lpad(m.cpf,11,'0') = lpad(v_cpf,11,'0')) then
    return jsonb_build_object('ok', false, 'motivo', 'SEM_EVIDENCIA_166_NO_ESPELHO',
      'explicacao', 'a linha do portador 166 precisa existir em prime_portador_membro antes de registrar a evidencia');
  end if;

  -- IDENTIDADE JA EXISTENTE NAO SE SOBRESCREVE CEGAMENTE. Se o pagamento ja tem
  -- aluno, o CPF que a Prime devolveu tem de ser o MESMO daquele aluno. Divergir
  -- significa que a registration do arquivo aponta para outra pessoa -- e nesse
  -- caso nao se toca em identidade nem se confirma nada.
  if v_pag.aluno_id is not null then
    ...
    if nullif(v_cpf_aluno,'00000000000') is not null
      ...
      return jsonb_build_object('ok', false, 'motivo', 'CPF_DIVERGE_DO_ALUNO_VINCULADO',
        'aluno_id', v_pag.aluno_id, 'explicacao',
        'identidade nao alterada e evidencia nao registrada -- conferir a quem pertence a registration');
    end if;
  end if;
...
-- ---------------------------------------------------------------------------
--
-- O modo pontual da Edge existe, mas alguem precisa chama-lo. Este e o menor
-- disparador possivel: reusa o padrao que `prime_portador_mutirao` ja usa desde
-- 24/08 -- segredos `projeto_url` e `prime_cadastro_token` do Vault,
-- `net.http_post` com `x-rotina-token`, e o disjuntor `sistema_sob_carga()`.
--
-- NUNCA DENTRO DO GATILHO. Quem chama isto e `conciliacao_reprocessar`, e so no
-- modo aplicado -- ou seja, a rodada horaria. O gatilho de INSERT chama
...
-- ---------------------------------------------------------------------------
-- 5b. A RODADA HORARIA PASSA A DISPARAR O CAMINHO B
-- ---------------------------------------------------------------------------
--
-- Unica mudanca em `conciliacao_reprocessar`: no fim, e SO no modo aplicado,
-- chama o disparador. O corpo restante e byte a byte o de 20260914170000.
-- O erro do disparo e engolido de proposito -- consulta ao Prime que falha nao
-- pode derrubar a conciliacao do lote inteiro.
...
comment on function public.conciliacao_reprocessar(boolean, int) is
  'Reprocessa os pagamentos PENDENTES e PROSPECTIVOS (status_conciliacao nao nulo e diferente de BAIXADO) pelo motor unico. Historico com status NULL fica intocado. No modo aplicado, dispara ao fim a consulta pontual ao portador para o que sobrou sem evidencia. p_aplicar=false e previa sem DML.';
...
begin
  ...
  -- o disparador existe, e nao e chamavel de fora
  if not exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
                  where n.nspname='public' and p.proname='conciliacao_consultar_portador_pendentes') then
    raise exception 'o disparador do caminho ao vivo nao foi criado';
  end if;
  if has_function_privilege('authenticated',
       'public.conciliacao_consultar_portador_pendentes(int)'::regprocedure, 'EXECUTE') then
    raise exception 'o disparador ficou chamavel por authenticated';
  end if;

  -- a rodada horaria dispara; o gatilho do INSERT nao
  if (select p.prosrc from pg_proc p join pg_namespace n on n.oid=p.pronamespace
       where n.nspname='public' and p.proname='conciliacao_reprocessar')
     not ilike '%conciliacao_consultar_portador_pendentes%' then
    raise exception 'a rodada horaria nao dispara o caminho ao vivo';
  end if;
  if (select p.prosrc from pg_proc p join pg_namespace n on n.oid=p.pronamespace
       where n.nspname='public' and p.proname='_pagamento_conciliar') ilike '%net.http%' then
    raise exception 'o gatilho de INSERT faz chamada externa';
  end if;
  if (select p.prosrc from pg_proc p join pg_namespace n on n.oid=p.pronamespace
       where n.nspname='public' and p.proname='pagamento_conciliar_um') ilike '%net.http%' then
    raise exception 'o motor faz chamada externa';
  end if;
