-- =============================================================================
-- TV ReATIVA — Etapa 3 (complemento): estado "aniversário HOJE"
-- =============================================================================
-- No dia exato do aniversário, a tela de Aniversariantes vira destaque
-- individual. A regra é calculada SÓ na geração do snapshot (data em
-- America/Sao_Paulo); a TV apenas exibe o estado gravado (payload.aniversariantes_hoje).
-- Dois aniversariantes no mesmo dia: ambos entram na lista (a TV mostra os dois).
-- No dia seguinte, a lista fica vazia e volta ao card mensal automaticamente.
-- Sem idade nem qualquer dado pessoal.
-- =============================================================================
...
begin
  ...
  -- Aniversariantes do mês (card mensal)
  ...
  -- Aniversariantes de HOJE (destaque individual; um ou mais nomes no mesmo dia)
  ...
  select jsonb_build_object('valor', round(valor_pago)::bigint,
    ...
    into v_maior_dia from public.pagamentos where data_pagamento = current_date order by valor_pago desc nulls last limit 1;
  select jsonb_build_object('operador', op, 'valor', vl) into v_top_rec_dia from (
    ...
    from public.pagamentos where data_pagamento = current_date and coalesce(nullif(operador_nome,''), operador_email) is not null
  ...
  v_rankings := jsonb_build_object(
    ...
    'melhor_mes', (select jsonb_build_object('operador', op) from (
        select upper(btrim(coalesce(nullif(operador_nome,''),operador_email))) op, sum(valor_pago) v, count(*) q
        from public.pagamentos where date_trunc('month',data_pagamento)=date_trunc('month',current_date) and coalesce(nullif(operador_nome,''),operador_email) is not null
        group by 1 order by v desc, q desc, op asc limit 1) a),
