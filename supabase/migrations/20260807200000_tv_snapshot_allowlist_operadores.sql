-- TV ReATIVA: restringe rankings/destaques por pessoa aos 8 operadores da equipe.
--
-- Contexto: os rankings de VALOR da TV (telas Hoje/Mês/top3/maior pagamento) eram
-- montados direto de public.pagamentos SEM allowlist. Pessoas de fora da equipe de
-- cobrança (financeiro e outras áreas) constam em pagamentos com operador_email NULL
-- e vazavam para o telão. O ranking de ACIONAMENTOS já usava a allowlist v_ops; aqui
-- aplicamos a MESMA allowlist (lower(operador_email) = any(v_ops)) a todos os blocos
-- de pagamentos por pessoa.
--
-- Escopo: TOTAIS (recuperado/honorários do dia e do mês, contagens, projeção, metas)
-- permanecem com a empresa inteira — o valor recuperado real inclui todos os pagamentos.
-- Apenas os rankings/destaques nominais passam a mostrar somente os 8 operadores.
-- Verificado em prod: nenhum dos 8 operadores tem pagamento com operador_email NULL,
-- então o filtro não descarta linha legítima da equipe.
