-- RECUPERACAO DE ACORDO PAGO SEM IMPORTACAO -- SO ACORDO A VISTA.
--
-- O PROBLEMA (diagnostico de 16/09/2026, PREMISSA 19). O Relatorio de Titulos
-- em Aberto so traz acordo que ainda tem titulo em aberto no instante da
-- extracao. Acordo a vista pago antes da extracao nunca chega a `acordos`, e o
-- pagamento dele fica em AGUARDANDO_ACORDO para sempre: o motor procura a
-- parcela pelo boleto e ela nao existe.
--
-- O QUE A MEDICAO MOSTROU. Nenhuma fonte ligada ao projeto devolve a
-- estrutura de um acordo ja pago: nem o relatorio, nem `/agreements`, nem o
-- extrato da Prime (portador 166 nunca vem; `de_acordo` nunca e verdadeiro).
-- Mas o acordo A VISTA tem uma parcela so -- e essa parcela esta inteira no
-- proprio pagamento: numero do acordo e da parcela no boleto, valor, vencimento
-- e operador. O que falta nao e dado: e alguem declarar que era a vista e quais
-- mensalidades ele quitou. Esta migration da essa declaracao a gestao, com
-- trava, simulacao e auditoria.
--
-- O QUE ESTA MIGRATION NAO TOCA, DE PROPOSITO:
--   * `pagamento_conciliar_um` (motor de baixa) -- a baixa continua sendo dele;
--   * `pagamento_vincular_aluno` e `vincular_titulos_acordo` -- sao CHAMADAS,
--     nao alteradas;
--   * `importar_acordos`, `parcelas_amarrar_boleto`, o cron e
--     `fluxo_pagamentos_config` -- a consulta ao portador segue desligada;
--   * nenhum pagamento, acordo, parcela ou titulo existente. A migration cria
--     funcoes e nao escreve dado nenhum.
--
-- AS TRES FUNCOES:
--   acordo_avista_previa ..... SQL puro e STABLE. Decide e descreve, sem DML.
--                              E o mesmo texto que roda em modo simulacao e
--                              dentro da confirmacao.
--   acordo_avista_registrar .. portao da gestao. p_confirmar = false devolve a
--                              previa. p_confirmar = true refaz a previa sob
--                              cadeado e so grava se TODAS as validacoes
--                              passarem; qualquer desvio do simulado aborta a
--                              transacao inteira.
--   pagamentos_trava ......... diz a tela em que ponto cada pagamento travou,
--                              para a acao oferecida ser a do ponto exato.
--
-- ORIGEM: RECUPERACAO_ACORDO_PAGO_SEM_IMPORTACAO. Vai na observacao do acordo e
-- da parcela, no vinculo do pagamento e na `auditoria`.
--
-- CREDITO. O responsavel do acordo e o OPERADOR DO PAGAMENTO, nunca quem da
-- gestao confirmou. `_acordo_herda_responsavel_do_aluno` so preenche quando o
-- campo vem nulo, e aqui ele vem preenchido. A baixa credita o mesmo operador:
-- o motor grava `confirmado_por_email = pagamentos.operador_email`.
--
-- A AUSENCIA PRECISA SER EXPLICADA. Se um acordo de numero MAIOR ja tinha
-- vindo numa importacao de dia ANTERIOR ao pagamento, este acordo ja existia
-- naquela extracao e nao apareceu -- a ausencia nao e "pago antes da
-- extracao". Medido: 71752 (pago em 16/09) contra 71757, importado em 14/09.
-- Esse caso fica FORA do fluxo normal: a previa recusa e a tela nao oferece a
-- acao. Nesta primeira versao nao existe liberacao manual de trava nenhuma.
--
-- MARGEM SEGURA DE VALOR (decisao da gestao, 16/09/2026). Soma das mensalidades
-- <= valor pago <= soma x 1,15. Nos quatro casos explicados do dia a razao foi
-- 1,1311 em todos; o unico perto de 1,30 foi o 71752, justamente a excecao nao
-- explicada. Acima de 1,15 a previa recusa e diz que a diferenca excede a
-- margem segura -- sem override.
...
aluno_mat as (
  select count(a.id) as n, min(a.id::text)::uuid as aluno_id
    from mat join public.alunos a
      on lpad(regexp_replace(coalesce(a.cpf, ''), '\D', '', 'g'), 11, '0') = mat.cpf11
   where mat.n_cpf = 1
),
aluno_nom as (
  select count(a.id) as n, min(a.id::text)::uuid as aluno_id
    from b join public.alunos a
...
comment on function public.acordo_avista_previa(uuid, uuid[]) is
  'Previa do registro de acordo a vista pago sem importacao (RECUPERACAO_ACORDO_PAGO_SEM_IMPORTACAO). SQL puro, sem DML: identifica o aluno por matricula+nome, confere o boleto, as mensalidades escolhidas e a regra de valor, e descreve o que seria criado. Nao e chamavel de fora: a porta e acordo_avista_registrar.';

revoke all on function public.acordo_avista_previa(uuid, uuid[]) from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. O REGISTRO -- SIMULA POR PADRAO, GRAVA SO COM TUDO APROVADO
-- ---------------------------------------------------------------------------
...
comment on function public.acordo_avista_registrar(uuid, uuid[], boolean) is
  'Registro de acordo a vista pago antes de ser importado (RECUPERACAO_ACORDO_PAGO_SEM_IMPORTACAO). Gestao apenas. p_confirmar=false (padrao) devolve a simulacao sem gravar. p_confirmar=true exige as mensalidades escolhidas, refaz a previa sob cadeado e grava so se tudo for aprovado: cria acordo e parcela com o boleto do pagamento, vincula as mensalidades (vincular_titulos_acordo) e entrega a baixa ao motor (pagamento_vincular_aluno). Estado final diferente do simulado aborta tudo.';

revoke all on function public.acordo_avista_registrar(uuid, uuid[], boolean) from public, anon;
grant execute on function public.acordo_avista_registrar(uuid, uuid[], boolean) to authenticated;

-- ---------------------------------------------------------------------------
-- 3. EM QUE PONTO O PAGAMENTO TRAVOU -- PARA A TELA OFERECER A ACAO CERTA
-- ---------------------------------------------------------------------------
--
--   ACORDO_AVISTA_AUSENTE ............ aluno identificado; boleto 0001 unico;
--                                      acordo nao existe -> Registrar acordo a vista
--   ACORDO_AVISTA_AUSENCIA_NAO_EXPLICADA  como acima, mas um acordo de numero
--                                      maior ja veio antes do dia do pagamento:
--                                      fora do fluxo normal -> sem acao
--   ACORDO_PARCELADO_AUSENTE ......... aluno identificado, mas ha outro boleto
--                                      do acordo ou nao e a parcela 1 -> sem acao
--   ACORDO_CHEGOU_AGUARDANDO_RODADA .. o acordo ja entrou; falta a rodada horaria
--   ALUNO_VINCULADO_SEM_PROVA_DUPLA .. o pagamento tem aluno, mas matricula e nome
--                                      nao o confirmam -> sem acao
--   IDENTIDADE_DIVERGENTE ............ matricula e nome apontam para alunos
--                                      diferentes -> Vincular aluno
--   ALUNO_NAO_IDENTIFICADO ........... nenhuma das duas fecha -> Vincular aluno
--   FORA_DO_ESCOPO ................... estado diferente de AGUARDANDO_ACORDO
...
revoke all on function public.pagamentos_trava(uuid[]) from public, anon;
grant execute on function public.pagamentos_trava(uuid[]) to authenticated;
...
declare v_src text;
begin
  -- a previa e SQL puro e STABLE: nao existe caminho de escrita dentro dela
  if (select p.prolang from pg_proc p join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname = 'acordo_avista_previa')
     <> (select oid from pg_language where lanname = 'sql') then
    raise exception 'acordo_avista_previa deixou de ser SQL puro';
  end if;
  if (select p.provolatile from pg_proc p join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname = 'acordo_avista_previa') <> 's' then
    raise exception 'acordo_avista_previa deixou de ser STABLE';
  end if;

  -- a previa nao e porta de entrada
  if has_function_privilege('authenticated', 'public.acordo_avista_previa(uuid, uuid[])'::regprocedure, 'EXECUTE') then
    raise exception 'acordo_avista_previa ficou chamavel por authenticated';
  end if;

  -- o registro nao baixa parcela por conta propria: a baixa e do motor
  select p.prosrc into v_src from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'acordo_avista_registrar';
  if v_src ilike '%update public.parcelas%' or v_src ilike '%update public.pagamentos%' then
    raise exception 'acordo_avista_registrar passou a escrever baixa ou pagamento por fora do motor';
  end if;
  if v_src not ilike '%public.pagamento_vincular_aluno(%' or v_src not ilike '%public.vincular_titulos_acordo(%' then
    raise exception 'acordo_avista_registrar deixou de usar as funcoes existentes de vinculo';
  end if;

  -- o portao vem antes de qualquer leitura
  if position('usuario_e_gestao' in v_src) > position('acordo_avista_previa' in v_src) then
    raise exception 'o portao da gestao nao e a primeira coisa do registro';
  end if;
end $prova$;
