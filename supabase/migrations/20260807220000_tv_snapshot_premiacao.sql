-- =============================================================================
-- TV ReATIVA — bloco "premiacao" no snapshot
-- -----------------------------------------------------------------------------
-- Acrescenta ao payload da TV as FAIXAS DE PREMIAÇÃO da competência corrente,
-- lidas de metas_projecao (M1..M4 = valor + percentual). A TV passa a exibir uma
-- tela dedicada de premiação (comissão desde o 1º pagamento até a máxima).
-- Fiel à função de produção (pg_get_functiondef 2026-08-07): idêntica, com 3
-- acréscimos cirúrgicos — declaração v_premiacao, cálculo do bloco e chave no
-- return. Nenhuma outra linha muda.
-- =============================================================================
...
begin
  ...
  -- ===== BLOCO NOVO: premiacao (faixas M1..M4 da competência corrente) =====
  -- Lê de metas_projecao. M1 com valor <= 1 = "ganha desde o 1º pagamento".
  -- Cada faixa leva o limite superior (proximo marco) para exibir a banda.
  select jsonb_build_object(
