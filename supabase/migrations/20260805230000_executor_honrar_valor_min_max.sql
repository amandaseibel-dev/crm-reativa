-- Ajuste: executor honra valor_min/valor_max dos filtros (paridade com a execução
-- imediata). Mantém todas as exclusões de segurança/revalidação. Só adiciona 2
-- condições de faixa de saldo no SELECT de elegibilidade.
  ...
  loop
    ...
    exception when others then
      if c.tentativa + 1 < v_maxtent then
        update public.acoes_massivas_agendamentos
           set status='AGENDADO', proxima_tentativa_em=now()+interval '5 minutes', erro_execucao=left(sqlerrm,500)
         where id=c.id;
      else
        update public.acoes_massivas_agendamentos
