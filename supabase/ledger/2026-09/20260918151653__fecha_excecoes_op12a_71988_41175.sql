-- FECHAMENTO DAS EXCECOES HUMANAS -- 18/09/2026 -- op12a_71988_41175
-- Autorizado pela Amanda em 18/09/2026: PAGAMENTO_POSTERIOR_APROPRIADO_EM_ACORDO_ANTIGO.
-- O pagamento 50719880001 renegociou/pagou o acordo antigo 41175; baixa oficial
-- (baixar_parcela_acordo) nas parcelas 50411750001 com o proprio
...
do $op$
declare
  c_pid constant uuid := '4c7f390a-3e8e-49e0-b5eb-80225d7b3e8e';
  c_alvos constant text[] := array['50411750001'];
...
begin
  ...
  if v_pag.id is null or ltrim(v_pag.numero_parcela_completo,'0') <> '50719880001' then raise exception 'OP_ABORTADA: pagamento nao encontrado'; end if;
  ...
  select id, aluno_id into v_acordo, v_aluno from public.acordos where numero_ulbra = '41175' and upper(status) = 'ATIVO';
  if v_acordo is null or (select count(*) from public.acordos where numero_ulbra = '41175') <> 1 then raise exception 'OP_ABORTADA: acordo 41175 nao unico/ATIVO'; end if;
  ...
  if (select count(distinct lpad(regexp_replace(cpf,'\D','','g'),11,'0')) from public.prime_contratos where registration = v_pag.matricula) <> 1
    ...
    raise exception 'OP_ABORTADA: aluno/CPF do pagamento nao e o do acordo 41175';
  end if;
  ...
  select 'op12a_71988_41175', 'parcelas', q.id, to_jsonb(q) from public.parcelas q where q.acordo_id = v_acordo
  union all select 'op12a_71988_41175', 'acordos', a.id, to_jsonb(a) from public.acordos a where a.id = v_acordo
  union all select 'op12a_71988_41175', 'fila_pagamento_sem_vinculo', f.pagamento_id, to_jsonb(f) from public.fila_pagamento_sem_vinculo f where f.pagamento_id = c_pid;
  ...
  for r in select id, valor from public.parcelas where acordo_id = v_acordo and boleto = any(c_alvos) order by vencimento loop
    ...
    update public.baixas_pagamento
       set observacao_operador = 'PAGAMENTO_POSTERIOR_APROPRIADO_EM_ACORDO_ANTIGO: pagamento 50719880001 (' || c_pid::text || ') de '
    ...
    update public.parcelas
       set observacao = coalesce(observacao,'') || case when coalesce(observacao,'') = '' then '' else ' | ' end
             || 'PAGAMENTO_POSTERIOR_APROPRIADO_EM_ACORDO_ANTIGO: pago pelo boleto 50719880001 em ' || to_char(v_pag.data_pagamento,'DD/MM/YYYY')
     where id = r.id;
  end loop;
  ...
  v_e := public.conciliacao_encerrar(c_pid, 'PAGAMENTO_POSTERIOR_APROPRIADO_EM_ACORDO_ANTIGO: pagamento 50719880001 apropriado nas parcelas '
           || array_to_string(c_alvos, ', ') || ' do acordo 41175 pela baixa oficial; sem acordo, parcela ou pagamento novo. Autorizado pela Amanda.');
  perform set_config('request.jwt.claims', '', true);
  ...
  values ('fechamento_pagamentos_20260918', 'PAGAMENTO_POSTERIOR_APROPRIADO_EM_ACORDO_ANTIGO', 'acordos', v_acordo,
          jsonb_build_object('pagamento_id', c_pid, 'boleto', '50719880001', 'acordo', '41175', 'parcelas', to_jsonb(c_alvos),
