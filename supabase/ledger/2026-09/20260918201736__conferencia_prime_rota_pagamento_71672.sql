-- CONFERENCIA PRIME -- rota financeira do pagamento 71672 (L6), autorizada pela gestao em 18/09/2026 depois da correcao C/D.
-- A diferenca 11/09 (Prime) x 14/09 (pagamento) isolada deixou de bloquear; a liquidacao tem de ser ate 5 dias antes do pagamento.
-- Travas extras: nenhum outro pagamento do aluno alem do 71672 e do 64776 (acordo 3638), acordo 3638 comprovadamente nao relacionado
-- (Prime 064776, QUITADO, parcela unica paga em 10/07, ja vinculado a outro titulo), titulo 4049973 ainda pendente.
...
do $rota$
declare
  c_bol   constant text   := '71672';
  c_pag   constant uuid   := 'a7255de3-9251-4233-94a4-f616e734e58c';
  c_tits  constant uuid[] := array['380fcfc5-e3c2-49a7-92c0-295ba5e930b5']::uuid[];
  c_sol   constant uuid   := 'fc804b10-1365-481a-a08e-9be29fe253c3';
  c_mot   constant text   := 'ROTA_FINANCEIRA_PAGAMENTO: Conferencia Prime, gestao 18/09/2026. O titulo segue pela rota do pagamento '
                             || '71672 (acordo a vista registrado pelo pagamento na mesma transacao).';
...
begin
  ...
  select count(*) into v_n
   ...
   where t.id = any(c_tits)
     ...
     and (d.evidencia ->> 'liquidado_em')::date between v_pag.data_pagamento - 5 and v_pag.data_pagamento
  ...
  -- 71672: acordo 3638 comprovadamente nao relacionado e nenhum pagamento ou acordo concorrente
  if not exists (select 1 from public.acordos a where a.numero_acordo = 3638 and a.aluno_id = v_pag.aluno_id and a.status = 'QUITADO'
                   and ltrim(coalesce(a.numero_ulbra,''),'0') = '64776'
                   and not exists (select 1 from public.acordo_titulo_vinculo v where v.acordo_id = a.id and v.titulo_id = any(c_tits))
                   and not exists (select 1 from public.parcelas p where p.acordo_id = a.id and upper(coalesce(p.status,'')) <> 'PAGO')) then
    raise exception 'ROTA_PRE %: acordo 3638 nao esta comprovadamente fora da relacao', c_bol;
  end if;
  if exists (select 1 from public.pagamentos p
              where (p.aluno_id = v_pag.aluno_id or lpad(regexp_replace(coalesce(p.cpf,''),'\D','','g'),11,'0') = lpad(regexp_replace(coalesce(v_pag.cpf,''),'\D','','g'),11,'0'))
                and p.id <> c_pag and ltrim(coalesce(p.numero_parcela_completo,''),'0') <> '50647760001') then
    raise exception 'ROTA_PRE %: ha outro pagamento do aluno', c_bol;
  end if;
  if exists (select 1 from public.acordos a where a.aluno_id = v_pag.aluno_id and a.numero_acordo <> 3638) then
    raise exception 'ROTA_PRE %: ha outro acordo do aluno', c_bol;
  end if;
  ...
  select a.id into v_proibido from public.acordos a where a.numero_acordo = 3638;
  ...
  values ('amanda.seibel@aelbra.com.br', 'CONFERENCIA_PRIME_ROTA_FINANCEIRA_PAGAMENTO', 'pagamentos', c_pag,
          jsonb_build_object('boleto', c_bol, 'grupo', 'L6', 'regra_de_data', 'liquidacao Prime 11/09 ate 5 dias antes do pagamento 14/09 (autorizado pela gestao)', 'pagamento_id', c_pag, 'acordo_id', v_acordo,
