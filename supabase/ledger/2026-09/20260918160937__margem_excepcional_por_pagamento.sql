-- MARGEM EXCEPCIONAL POR PAGAMENTO, AUTORIZADA PELA GESTAO -- 18/09/2026.
--
-- A margem segura de 1,15 da previa do acordo a vista NAO muda. Esta migration
-- cria so uma autorizacao UM A UM: a gestao chama
-- pagamento_autorizar_margem_excepcional(pagamento, motivo), que recalcula na
-- hora e so grava a autorizacao se:
--   - a margem e o UNICO bloqueio da previa;
--   - o honorario do pagamento e 8% da base (pago - honorario);
--   - todas as mensalidades selecionadas tem valor no extrato do Prime;
--   - valor das mensalidades <= base <= valor corrigido do Prime.
-- A previa aceita DIFERENCA_DENTRO_DA_MARGEM_SEGURA so com a linha daquele
-- pagamento e so se o valor pago e o conjunto de mensalidades continuarem os
-- mesmos da autorizacao. Nada automatico: sem a linha, tudo como antes.
-- Auditoria: AUTORIZACAO_GESTAO_MARGEM_EXCEPCIONAL (na autorizacao) e
-- margem_excepcional no registro do acordo.
--
-- Rollback: supabase/rollbacks/20260918170000_margem_excepcional_por_pagamento.rollback.sql

create table if not exists public.pagamento_margem_autorizada (
  pagamento_id uuid primary key references public.pagamentos(id),
  valor_pago numeric not null,
  valor_honorario numeric not null,
  base numeric not null,
  soma_titulos numeric not null,
  titulo_ids uuid[] not null,
  valor_prime numeric not null,
...
alter table public.pagamento_margem_autorizada enable row level security;
revoke all on public.pagamento_margem_autorizada from anon, authenticated;

create or replace function public.pagamento_autorizar_margem_excepcional(p_pagamento_id uuid, p_motivo text)
 returns jsonb
 language plpgsql
 security definer
...
declare
  ...
  v_sem_prime int;
begin
  -- Portao interno: EXECUTE fica com authenticated, a decisao e da gestao.
  if not coalesce(public.usuario_e_gestao(), false) then
    raise exception 'Autorizar margem excepcional é decisão da gestão financeira.' using errcode = '42501';
  end if;
  if length(btrim(coalesce(p_motivo, ''))) < 10 then
    return jsonb_build_object('ok', false, 'erro', 'MOTIVO_OBRIGATORIO');
  end if;
  select p.* into v_pag from public.pagamentos p where p.id = p_pagamento_id;
  if v_pag.id is null then return jsonb_build_object('ok', false, 'erro', 'PAGAMENTO_NAO_ENCONTRADO'); end if;
  if coalesce(v_pag.status_conciliacao, '') <> 'AGUARDANDO_ACORDO'
     or not exists (select 1 from public.fila_pagamento_sem_vinculo f
                     where f.pagamento_id = p_pagamento_id and f.decisao is null) then
    return jsonb_build_object('ok', false, 'erro', 'PAGAMENTO_NAO_PENDENTE');
  end if;

  -- A previa do botao decide tudo; a margem tem de ser o UNICO bloqueio.
  v_previa := public.acordo_avista_previa(p_pagamento_id, null);
  if v_previa -> 'bloqueios' <> '["DIFERENCA_DENTRO_DA_MARGEM_SEGURA"]'::jsonb then
    return jsonb_build_object('ok', false, 'erro', 'OUTRO_BLOQUEIO_ALEM_DA_MARGEM', 'bloqueios', v_previa -> 'bloqueios');
  end if;
  select array_agg((t ->> 'id')::uuid order by (t ->> 'id')), sum((t ->> 'valor')::numeric)
    into v_ids, v_soma
    from jsonb_array_elements(v_previa -> 'titulos' -> 'selecionados') t
   where t ->> 'impedimento' is null;

  -- Honorario de 8% sobre a base (faixa observada em todos os pagamentos baixados).
  v_hon := coalesce(v_pag.valor_honorario, 0);
  v_base := v_pag.valor_pago - v_hon;
  if v_hon <= 0 or v_base <= 0 or abs(v_hon / v_base - 0.08) > 0.0075 then
    return jsonb_build_object('ok', false, 'erro', 'HONORARIO_FORA_DE_8_POR_CENTO', 'honorario', v_hon, 'base', v_base);
  end if;

  -- Evidencia do Prime: valor corrigido (ou bruto, o maior) de CADA mensalidade.
  select count(*) filter (where e.boleto is null),
         sum(greatest(coalesce(e.valor_pago, 0), coalesce(e.valor_bruto, 0)))
    into v_sem_prime, v_prime
    from public.acordos_titulos t
    left join lateral (select x.boleto, x.valor_pago, x.valor_bruto from public.prime_extrato x
                        where ltrim(x.boleto, '0') = ltrim(t.documento, '0')
                        order by x.coletado_em desc limit 1) e on true
   where t.id = any(v_ids);
  if v_sem_prime > 0 or coalesce(v_prime, 0) <= 0 then
    return jsonb_build_object('ok', false, 'erro', 'SEM_VALOR_DO_PRIME_PARA_TODAS_AS_MENSALIDADES');
  end if;
  -- A base tem de ficar entre o valor das mensalidades e o valor do Prime.
  if v_base < v_soma - 0.05 or v_base > v_prime + 0.005 then
    return jsonb_build_object('ok', false, 'erro', 'BASE_FORA_DO_INTERVALO_COMPROVADO',
                              'base', v_base, 'soma_titulos', v_soma, 'valor_prime', v_prime);
  end if;

  insert into public.pagamento_margem_autorizada
    (pagamento_id, valor_pago, valor_honorario, base, soma_titulos, titulo_ids, valor_prime, diferenca, motivo, autorizado_por)
  values (p_pagamento_id, v_pag.valor_pago, v_hon, v_base, v_soma, v_ids, v_prime, round(v_base - v_soma, 2),
          btrim(p_motivo), coalesce(nullif(v_email, ''), 'gestao'))
  on conflict (pagamento_id) do nothing;
  if not found then return jsonb_build_object('ok', true, 'ja_autorizado', true); end if;

  insert into public.auditoria (usuario, acao, tabela_afetada, registro_id, detalhes)
  values (coalesce(nullif(v_email, ''), 'gestao'), 'AUTORIZACAO_GESTAO_MARGEM_EXCEPCIONAL', 'pagamentos', p_pagamento_id,
          jsonb_build_object('pagamento_id', p_pagamento_id, 'valor_pago', v_pag.valor_pago, 'honorario', v_hon,
                             'base_sem_honorario', v_base, 'valor_titulos', v_soma, 'titulo_ids', to_jsonb(v_ids),
                             'valor_prime', v_prime, 'diferenca', round(v_base - v_soma, 2),
                             'razao_pago_sobre_titulos', round(v_pag.valor_pago / v_soma, 4),
                             'autorizado_por', coalesce(nullif(v_email, ''), 'gestao'), 'motivo', btrim(p_motivo)));

  return jsonb_build_object('ok', true, 'pagamento_id', p_pagamento_id, 'base', v_base, 'soma_titulos', v_soma,
                            'valor_prime', v_prime, 'diferenca', round(v_base - v_soma, 2));
end;
$fn$;
revoke all on function public.pagamento_autorizar_margem_excepcional(uuid, text) from public, anon;
grant execute on function public.pagamento_autorizar_margem_excepcional(uuid, text) to authenticated;
comment on function public.pagamento_autorizar_margem_excepcional(uuid, text) is
  'Gestao autoriza UM pagamento acima da margem 1,15, se a margem for o unico bloqueio, o honorario for 8% e a base (pago - honorario) ficar entre o valor das mensalidades e o valor corrigido do Prime. A margem global nao muda.';
...
-- ORIGEM EXTERNA AUTORIZADA (18/09/2026). Operador que nao esta no CRM so passa
-- com autorizacao da gestao PARA ESTE PAGAMENTO (pagamento_origem_externa_autorizada),
-- com o mesmo nome de origem do arquivo. Sem a linha, OPERADOR_CADASTRADO
-- continua bloqueando -- inclusive na recuperacao automatica.
ext as (
  select x.operador_origem, x.autorizado_por, x.autorizado_em, x.motivo
    from b join public.pagamento_origem_externa_autorizada x on x.pagamento_id = b.id
   where b.operador_email is null
     and coalesce(trim(b.operador_nome), '') <> ''
     and x.operador_origem = trim(b.operador_nome)
     and (select n from op) = 0
),
...
-- MARGEM EXCEPCIONAL AUTORIZADA (18/09/2026). A margem de 1,15 continua a
-- regra. So passa acima dela o pagamento que a gestao autorizou UM A UM
-- (pagamento_margem_autorizada), e so enquanto o valor pago e o conjunto de
-- mensalidades forem exatamente os da autorizacao.
marg as (
  select m.valor_pago, m.valor_honorario, m.base, m.soma_titulos, m.valor_prime, m.diferenca,
         m.autorizado_por, m.autorizado_em
    from b join public.pagamento_margem_autorizada m on m.pagamento_id = b.id
   where m.valor_pago = b.valor_pago
     and m.soma_titulos = (select soma from tot)
     and (select coalesce(array_agg(id order by id), '{}'::uuid[]) from sel)
         = (select coalesce(array_agg(x order by x), '{}'::uuid[]) from unnest(m.titulo_ids) x)
),
dup as (
  select count(*) as n
  ...
  select x.ordem, x.codigo, coalesce(x.ok, false) as ok, x.detalhe
    from (values
      ...
      (16, 'OPERADOR_CADASTRADO',
          (select n = 1 from op) or (select count(*) = 1 from ext),
          'operador do pagamento: ' || coalesce((select b.operador_email from b), '(sem)')
          || coalesce((select ' · origem externa ' || operador_origem || ' autorizada por ' || autorizado_por
                              || ': crédito individual não atribuído' from ext), '')),
      ...
      (20, 'DIFERENCA_DENTRO_DA_MARGEM_SEGURA',
          ...
          coalesce(
            ...
            || coalesce((select ' · margem excepcional autorizada por ' || autorizado_por
                                || ' (base ' || base::text || ', Prime ' || valor_prime::text || ')' from marg), ''),
...
select jsonb_build_object(
  ...
  'margem_excepcional', (select jsonb_build_object('valor_pago', valor_pago, 'valor_honorario', valor_honorario,
                                                    'base', base, 'soma_titulos', soma_titulos, 'valor_prime', valor_prime,
                                                    'diferenca', diferenca, 'autorizado_por', autorizado_por,
                                                    'autorizado_em', autorizado_em) from marg),
  'origem_externa', (select jsonb_build_object('operador_origem', operador_origem, 'autorizado_por', autorizado_por,
                                                'autorizado_em', autorizado_em, 'motivo', motivo,
                                                'credito_individual', 'NAO_ATRIBUIDO') from ext),
  ...
  'efeitos', (select jsonb_agg(e order by o) from (
      ...
      select 3, 'o crédito da baixa e a responsabilidade do acordo ficam com '
                || coalesce((select op.email from op),
                         (select 'ninguém -- origem externa ' || operador_origem || ', crédito individual não atribuído' from ext),
...
begin
  ...
  -- ORIGEM EXTERNA AUTORIZADA (18/09/2026): so existe quando a previa achou a
  -- autorizacao da gestao para ESTE pagamento. O acordo nasce sem credito
  -- individual e o gatilho de heranca de responsavel nao o preenche.
  v_externa := v_previa -> 'origem_externa';
  if jsonb_typeof(v_externa) is distinct from 'object' then v_externa := null; end if;
  v_marca := 'RECUPERACAO_ACORDO_PAGO_SEM_IMPORTACAO | pagamento ' || p_pagamento_id::text
          ...
          || case when v_externa is not null
                  then ' | ORIGEM EXTERNA: ' || (v_externa ->> 'operador_origem')
                       || ' | CREDITO INDIVIDUAL NAO ATRIBUIDO (autorizado por ' || (v_externa ->> 'autorizado_por') || ')'
                  else '' end;
  ...
  -- Origem externa: responsavel NULO, e a chave abaixo (so com esta funcao na
  -- pilha) impede a heranca do dono do aluno.
  if v_externa is not null then
    perform set_config('reativa.acordo_sem_credito', 'on', true);
  end if;
  ...
  perform set_config('reativa.acordo_sem_credito', 'off', true);
  ...
  values (v_email, 'RECUPERACAO_ACORDO_PAGO_SEM_IMPORTACAO', 'acordos', v_acordo_id,
          jsonb_build_object(
            ...
            'origem_externa', v_externa,
            'margem_excepcional', case when jsonb_typeof(v_previa -> 'margem_excepcional') = 'object'
                                       then v_previa -> 'margem_excepcional' end,
            'credito_individual', case when v_externa is not null then 'NAO_ATRIBUIDO' else 'OPERADOR_DO_PAGAMENTO' end,
            'estado_antes', v_previa,
            'estado_depois', v_depois));

  if jsonb_typeof(v_previa -> 'margem_excepcional') = 'object' then
    update public.pagamento_margem_autorizada
       set usado_em = now(), acordo_id = v_acordo_id
     where pagamento_id = p_pagamento_id;
  end if;

  if v_externa is not null then
    update public.pagamento_origem_externa_autorizada
       set usado_em = now(), acordo_id = v_acordo_id
     where pagamento_id = p_pagamento_id;
  end if;
