-- VINCULO DA MENSALIDADE: HISTORICO INATIVO NAO IMPEDE A AMARRA
-- 18/09/2026.
--
-- O DEFEITO (latente, exposicao zero em 18/09/2026: 21 acordos a vista, 23
-- mensalidades, 23 vinculos ativos). `vincular_titulos_acordo` so criava a
-- linha em acordo_titulo_vinculo quando a mensalidade nao tinha NENHUMA linha
-- na tabela. Uma linha INATIVA de um acordo antigo bastava para a insercao ser
-- pulada: a mensalidade ficava NEGOCIADO/PAGO com `acordo_id` preenchido, sem
-- vinculo ativo, e a funcao respondia ok. O registrador do acordo a vista
-- conferia `vinculados` -- que conta mensalidades atualizadas, nao vinculos --
-- e tambem nao via a falta. A previa do a vista declara elegivel a mensalidade
-- que so tem historico inativo, entao o caminho existia.
--
-- A REGRA, por mensalidade (vincular_titulos_acordo):
--   1. sem vinculo ativo: cria o vinculo ativo com o acordo atual;
--   2. vinculo INATIVO antigo: o historico fica intocado e nasce o ativo novo;
--   3. vinculo ativo com o MESMO acordo: idempotente, nada duplica;
--   4. vinculo ativo com OUTRO acordo: a operacao inteira volta sem escrever;
--      nao move, nao desativa;
--   5. nunca dois vinculos ativos (o indice unico ux_titulo_vinculo_ativo
--      continua como guarda adicional).
-- E a prova no fim: toda mensalidade pedida sai com exatamente um vinculo
-- ativo com este acordo, ou a transacao inteira volta.
--
-- acordo_avista_registrar: depois de vincular, e de novo no estado final,
-- exige para CADA mensalidade escolhida exatamente um vinculo ativo com o
-- acordo criado. Faltou, RAISE: nada fica gravado.
--
-- O QUE NAO MUDA: a previa do a vista, o motor de baixa, a porta de maquina,
-- permissoes (create or replace preserva o ACL), a regra de elegibilidade da
-- mensalidade nova (PARCELAS_INELEGIVEIS), o estado que a mensalidade assume
-- (NEGOCIADO/vinculada ou PAGO/quitada). Chamadores de vincular_titulos_acordo
-- em producao: acordo_avista_registrar e as telas FinanceiroAluno e
-- VincularMensalidadesAcordo, que ja tratam `ok = false` pelo campo `erro`.
-- Os dois erros novos saem do mesmo jeito: TITULO_NAO_ENCONTRADO e
-- TITULO_COM_VINCULO_ATIVO_EM_OUTRO_ACORDO.
--
-- Unica mudanca de comportamento alem do defeito: mensalidade com `acordo_id`
-- deste acordo e SEM a linha do vinculo deixava de ser tocada (contava como
-- "ja estava"); agora ganha a linha que faltava (regra 1), sem mudar
-- situacao nem status, e aparece em `amarras_refeitas`.
--
-- Rollback: supabase/rollbacks/20260918120000_vinculo_titulo_ignora_historico_inativo.rollback.sql
-- (devolve os dois corpos de producao: registrar 391bbee9, vincular bfad7027).

-- ---------------------------------------------------------------------------
-- 1. vincular_titulos_acordo: a regra por mensalidade
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. acordo_avista_registrar: a amarra provada antes de aceitar
-- ---------------------------------------------------------------------------
