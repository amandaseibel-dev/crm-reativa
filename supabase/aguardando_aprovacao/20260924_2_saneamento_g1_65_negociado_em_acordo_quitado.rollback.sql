-- ============================================================================
-- ROLLBACK do arquivo 2 (G1, 65 titulos) -- por ID, contra o ESTADO DEIXADO
-- ============================================================================
-- Restaura o que foi gravado em `_backup_saneamento_g1_20260924`, titulo por
-- titulo.
--
...
--       Aqui isto pesa: a restauracao devolve o titulo para NEGOCIADO, e se
--       entrou pagamento de verdade no meio, devolver seria "des-pagar".
--
...
begin
  ...
  select count(*) into v_sem_foto from public._backup_saneamento_g1_20260924
   where registro_pos is null or audit_log_id_pos is null;
  if v_sem_foto > 0 then
    raise exception 'ABORTA: % linha(s) sem fotografia posterior. O saneamento foi rodado por uma versao antiga do arquivo 2, sem o bloco 5. Sem ela nao ha com o que comparar, e este rollback NAO vai adivinhar -- reverta a mao, titulo por titulo.', v_sem_foto;
  end if;
end $$;
...
select b.titulo_id,
       case
         ...
         when exists (
           ...
           'apareceu pagamento proprio em pagamentos -- restaurar para NEGOCIADO seria des-pagar'
         when exists (
...
update public.acordos_titulos t
 ...
 where t.id = b.titulo_id
   and t.id in (select titulo_id from _g1_rev where motivo_recusa is null);
...
begin
  select count(*) into v_rev from _g1_rev where motivo_recusa is null;
  select count(*) into v_rec from _g1_rev where motivo_recusa is not null;

  select count(*) into v_falha
    from _g1_rev r
    join public._backup_saneamento_g1_20260924 b on b.titulo_id = r.titulo_id
