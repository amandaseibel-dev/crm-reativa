-- ROLLBACK de 20260725120000_projecao_amanda_adm_admin_e_filtro_unidade.sql
-- Restaura public.projecao_dashboard(text, text) EXATAMENTE como estava antes
-- (sem o parâmetro p_unidade, sem a visão administrativa da Amanda ADM).
-- Remove a versão de 3 parâmetros criada pela migration.

DROP FUNCTION IF EXISTS public.projecao_unidades_disponiveis();
DROP FUNCTION IF EXISTS public.projecao_dashboard(text, text, text);

CREATE OR REPLACE FUNCTION public.projecao_dashboard(p_mes text, p_operador_email text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.projecao_dashboard(text, text) TO authenticated, service_role;
