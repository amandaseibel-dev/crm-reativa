-- ============================================================================
-- ROLLBACK COMPLETO de 20260726180000_bloquear_anon_definer_projecao_suspeitas.sql
-- ============================================================================
--
-- !!! AVISO DE SEGURANÇA / LGPD — LEIA ANTES DE EXECUTAR !!!
-- Este script RESTAURA O ESTADO VULNERÁVEL ANTERIOR à migration: reintroduz
-- EXATAMENTE os 5 alertas `anon_security_definer_function_executable` do Security
-- Advisor, tornando as 5 funções SECURITY DEFINER (incluindo dados financeiros e
-- a função de trigger) novamente EXECUTÁVEIS pelo papel `anon` via /rest/v1/rpc/*.
-- Só pode ser usado em EMERGÊNCIA de compatibilidade e com AUTORIZAÇÃO EXPLÍCITA
-- da gestão de segurança. Reative a proteção assim que possível.
--
-- Diferente da versão anterior (que só reconcedia grants), este rollback restaura
-- por completo o estado pré-migration:
--   * corpo/definição ORIGINAL das 4 funções alteradas (remove o gate de identidade);
--   * LANGUAGE original (unidades e suspeitas voltam a LANGUAGE sql);
--   * SECURITY DEFINER, search_path='public' e owner=postgres originais
--     (CREATE OR REPLACE preserva owner=postgres, que já era o dono);
--   * grants originais (PUBLIC/authenticated/service_role) nas 5 funções.
-- A função de trigger public.trg_detectar_suspeita_duplicidade() NÃO teve o corpo
-- alterado pela migration (só grants), portanto aqui apenas seus grants são
-- restaurados — o corpo permanece intacto.
--
-- NÃO altera dados, triggers, decisões, pagamentos, suspeitas nem auditoria.
-- Este arquivo fica FORA de supabase/migrations (não é aplicado automaticamente).
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) projecao_dashboard  -> definição ORIGINAL (plpgsql, SECURITY DEFINER, sem gate)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2) projecao_unidades_disponiveis  -> definição ORIGINAL (LANGUAGE sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3) projecao_suspeitas_pagamentos_duplicados  -> definição ORIGINAL (LANGUAGE sql)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) registrar_decisao_suspeita_duplicidade  -> definição ORIGINAL (plpgsql, sem gate)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5) GRANTS ORIGINAIS (baseline pré-migration): PUBLIC + authenticated +
--    service_role + postgres nas 5 funções (inclui a função de trigger, cujo
--    corpo NÃO foi alterado pela migration). Reabre a exposição anônima.
-- ----------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION public.projecao_dashboard(text,text,text) TO PUBLIC, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.projecao_unidades_disponiveis() TO PUBLIC, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.projecao_suspeitas_pagamentos_duplicados() TO PUBLIC, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.registrar_decisao_suspeita_duplicidade(uuid,text,uuid,uuid,text) TO PUBLIC, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.trg_detectar_suspeita_duplicidade() TO PUBLIC, authenticated, service_role;
