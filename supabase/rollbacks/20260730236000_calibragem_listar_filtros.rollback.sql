-- ROLLBACK: CALIBRAGEM — FILTROS no drill-down (volta à versão de 5 args)
begin;
drop function if exists public.calibragem_listar_casos(text,text,text,integer,integer,jsonb);
create or replace function public.calibragem_listar_casos(
  p_operador_email text, p_indicador text, p_faixa text default null, p_ano integer default null, p_limit integer default 300
) returns jsonb language plpgsql security definer set search_path = public as $$
declare v_rows jsonb; v_fa_min int; v_fa_max int;
begin
  ...
  from (
    select jsonb_build_object('caso_id', c.id, 'aluno_id', c.aluno_id, 'cpf', coalesce(c.cpf_limpo, c.cpf),
             ...
             'ano_divida', extract(year from s.venc_min)) as t
  ...
  return jsonb_build_object('operador_email', p_operador_email, 'indicador', p_indicador, 'faixa', p_faixa, 'ano', p_ano, 'total', jsonb_array_length(v_rows), 'casos', v_rows);
end; $$;
