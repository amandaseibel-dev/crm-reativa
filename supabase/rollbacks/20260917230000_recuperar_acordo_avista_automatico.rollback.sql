-- ROLLBACK de 20260917230000_recuperar_acordo_avista_automatico.sql
--
-- Devolve os tres corpos exatos de producao de 17/09/2026 (md5 conferido no
-- banco: acordo_avista_registrar beb45df8, fluxo_pagamentos_rodar 07044729,
-- _pagamentos_baixar_lote 6df42e17, vincular_titulos_acordo 47125b28,
-- pagamento_vincular_aluno 341df31b), remove as duas funcoes novas e apaga a
-- chave da etapa. Nao mexe na previa, no motor de baixa nem em dado gravado:
-- acordo recuperado enquanto a etapa esteve ligada continua valendo.
...
drop function if exists public.acordo_avista_recuperar_pendentes(integer);
drop function if exists public.acordo_avista_recuperar_um(uuid, boolean);

delete from public.fluxo_pagamentos_config where etapa = 'recuperar_acordo_avista';
