...
export default function VincularMensalidadesAcordo({ alunoId }) {
  ...
  async function carregar() {
    ...
    const [{ data: acs }, { data: tits }] = await Promise.all([
      ...
      supabase.from("acordos_titulos").select("id, competencia, vencimento, valor_em_aberto, valor_original, saldo_corrigido, situacao, acordo_id").eq("aluno_id", alunoId).eq("situacao", "ABERTO").order("vencimento", { ascending: true }),
    ]);
    const lista = acs || [];
    setAcordos(lista);
    setTitulos(tits || []);
