...
export default function AjustesAlunoUnificado({ aluno, casos = [], onAtualizar }) {
  ...
  const podeAjustar = podeBaixarPagamento(emailUsuario);

  if (usuario && !podeAjustar) {
    return (
      <div style={styles.card}>
        <h2 style={styles.titulo}>Ajustes de Cadastro e Valores</h2>
        <p style={styles.texto}>
          Ajustes disponíveis somente para Amanda gestora.
        </p>
      </div>
    );
  }
...
const styles = {
  card: { background: "var(--rv-superficie)", borderRadius: "14px", padding: "18px", marginBottom: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", borderLeft: "6px solid #6f42c1" },
  titulo: { margin: 0, color: "var(--rv-tinta)" },
  texto: { margin: "6px 0 14px 0", color: "var(--rv-texto)" },
  bloco: { background: "var(--rv-fundo-cartao)", border: "1px solid var(--rv-borda)", borderRadius: "12px", padding: "14px", marginTop: "14px" },
  subtitulo: { margin: "0 0 12px 0", color: "var(--rv-tinta)" },
  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: "10px" },
  input: { padding: "11px", borderRadius: "8px", border: "1px solid var(--rv-borda-forte)", fontSize: "14px", width: "100%", boxSizing: "border-box" },
  ...
  caso: { background: "var(--rv-superficie)", border: "1px solid var(--rv-borda)", borderRadius: "10px", padding: "12px", marginBottom: "10px", display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center" },
  info: { margin: "4px 0", color: "var(--rv-texto)" },
  ...
  modalBox: { background: "var(--rv-superficie)", borderRadius: "14px", padding: "20px", width: "min(620px, 92vw)", boxShadow: "0 10px 30px rgba(0,0,0,0.25)" },
