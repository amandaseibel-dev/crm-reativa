...
export default function App() {
  ...
  { rota: "/executivo", label: "📊 Visão Executiva" },
    ...
    { rota: "/importar-acordos", label: "Importar Acordos", icone: "Upload", secao: "Gestão" }, { rota: "/acordos-duplicados", label: "Acordos duplicados", icone: "Copy", secao: "Gestão" }, { rota: "/pagamentos-sem-aluno", label: "Pagamentos sem aluno", icone: "UserSearch", secao: "Gestão" }, { rota: "/quitacao-sugerida", label: "Quitação sugerida", icone: "CheckCheck", secao: "Gestão" }, { rota: "/conferencia-prime", label: "Conferência Prime", icone: "ScanSearch", secao: "Gestão" }, { rota: "/ferramentas", label: "Ferramentas", icone: "FileStack", secao: "Gestão" }, { rota: "/importacoes", label: "Importações", icone: "Upload", secao: "Configurações" },
  ...
  const menu = menuBase.filter((item) => {
    ...
    if (item.rota === "/minha-agenda") { const emMa = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return emMa === "amanda.seibel@aelbra.com.br"; } if (item.rota === "/envio-gmail") { const emEg = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emEg); } if (item.rota === "/acordos-duplicados") { const emAd = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emAd); } if (item.rota === "/pagamentos-sem-aluno") { const emPs = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emPs); } if (item.rota === "/quitacao-sugerida") { const emQs = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emQs); } if (item.rota === "/conferencia-prime") { const emCp = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emCp); } if (item.rota === "/importar-acordos") { const emIa = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emIa); } if (item.rota === "/ferramentas") { const emFe = String(usuario?.perfil?.email || usuario?.auth?.email || "").toLowerCase().trim(); return ["amanda.seibel@aelbra.com.br","cobranca04@aelbra.com.br","cobranca07@aelbra.com.br"].includes(emFe); } return podeAcessar(perfil, item.rota);
  });
  return (
    <BrowserRouter>
